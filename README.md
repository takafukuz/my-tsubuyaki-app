...
つぶやきアプリ（管理画面を含む）のサンプル Java Web アプリケーション。
このリポジトリは主に JSP + Servlet（Maven プロジェクト）で構成されています。

## 概要
- 言語: Java
- ビルド: Maven
- 実行環境: Tomcat などの Java Servlet コンテナ
- レイアウト: JSP（`WEB-INF/jsp`）と CSS（`webapp/css`）
